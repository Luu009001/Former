...
class Multi_GTU(nn.Module):
    ...
    def forward(self, X):
        ...
        x_gtu.append(self.gtu0(X))  # 执行第一个尺度的1D因果卷积: (B,C,N,T-K1+1) == (B,C,N,T-2)   torch.Size([32, 128, 3, 13])
        x_gtu.append(self.gtu1(X))  # 执行第二个尺度的1D因果卷积: (B,C,N,T-K2+1) == (B,C,N,T-4)   torch.Size([32, 128, 3, 11])
        x_gtu.append(self.gtu2(X))  # 执行第三个尺度的1D因果卷积: (B,C,N,T-K3+1) == (B,C,N,T-6)   torch.Size([32, 128, 3, 9])
        time_conv = torch.cat(x_gtu, dim=-1)  # 拼接三个尺度的因果卷积层的输出: (B,C,N,3T+3-K1-K2-K3) = (B,C,N,3T-12)  torch.Size([32, 128, 3, 33])
        ...
        if self.pool:
            time_conv = self.pooling(time_conv) # (B,C,N,(3T+3-K1-K2-K3)/2) = (B,F,N,(3T-12)/2)  T==12时,池化后: (B,C,N,12)  torch.Size([32, 128, 3, 16])
