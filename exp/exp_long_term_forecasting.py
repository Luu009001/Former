...
class Exp_Long_Term_Forecast(Exp_Basic):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                if self.args.use_amp:
                    ...
                    if self.args.model in ['CARD', 'Proposed_SEA','Proposed_MGTUgd','Proposed_ASBMGTUgd','Proposed_ASBSEA','Proposed_SEA_MGTUgd_ASB','Proposed_ASB','Proposed_robdecomp','Proposed_SEA_MGTUgd_robdecomp','Proposed_SEACRMSA','Proposed_SEAMGTUgd','Proposed_SEAgdMGTUgd','Proposed_SEAgdAMGTU','Proposed_SEACRMSAgd']:
                        ...
                        batch_y = batch_y *self.ratio  #分别乘以 ratio，使得不同时间步的输出和目标带有不同的权重。
            ...
            # if self.args.model == 'CARD':
            if self.args.model in ['CARD', 'Proposed_SEA','Proposed_MGTUgd','Proposed_ASBSEA','Proposed_ASBMGTUgd','Proposed_SEA_MGTUgd_ASB','Proposed_ASB','Proposed_robdecomp','Proposed_SEA_MGTUgd_robdecomp','Proposed_SEACRMSA','Proposed_SEAMGTUgd','Proposed_SEAgdMGTUgd','Proposed_SEAgdAMGTU','Proposed_SEACRMSAgd']:
            ...
            # if self.args.lradj != 'TST': 
            if self.args.model in ['CARD', 'Proposed_SEA','Proposed_MGTUgd','Proposed_ASBMGTUgd','Proposed_ASBSEA','Proposed_SEA_MGTUgd_ASB','Proposed_ASB','Proposed_SEAgdMGTUgd','Proposed_SEA_MGTUgd_robdecomp','Proposed_SEACRMSA','Proposed_robdecomp','Proposed_SEAgdAMGTU','Proposed_SEAMGTUgd','Proposed_SEACRMSAgd']:
    ...
    def test(self, setting, test=0):
        ...
        if test:
            ...
            # self.model.load_state_dict(torch.load(os.path.join('./Record/Model_Save/Proposed_SEA_MGTUgd_ASB_liugongdao_MS_rjy_120_48_48_inverse_Itr0' + setting, 'checkpoint.pth')))
        ...
        # # wandb.log({"test mae": mae, "test mse": mse, "test rmse": rmse, "test mape": mape, "test mspe": mspe})
        ...
        # result save
        folder_path = './Record/results/' + setting + '/'
        if not os.path.exists(folder_path):
            os.makedirs(folder_path)
        mse,mae, rmse, mape, mspe ,r2,smap,t_u= metric(preds, trues)

        print('mse:{}, mae:{},rmse:{},mape:{},mspe={},R2{},SMAP:{}'.format(mse, mae, rmse, mape, mspe,r2,smap))
        f = open("long_term_forecast.txt", 'a')
        f.write(setting + "  \n")
        f.write('mse:{}, mae:{},rmse:{},mape:{},mspe={},R2{},SMAP:{}'.format(mse, mae, rmse, mape, mspe,r2,smap))
        f.write('\n')
        f.write('\n')
        f.close()

        # np.save(folder_path + 'metrics.npy', np.array([mse, mae, rmse, mape, mspe,r2,smap,kge,sde,t_u]))
        np.save(folder_path + 'metrics.npy', np.array([mse, mae, rmse, mape, mspe,r2,smap,t_u]))
