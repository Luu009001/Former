...
class Model(nn.Module):  #add2
    ...
    def forward(self, x, *args, **kwargs):    
        ...
        for i in range(self.k):
            ...
            x = x.permute(0,2,1)
            out= self.model(x,mask = mask,patch_len=period).permute(0,2,1)  #torch.Size([32, 48, 3])
            x = x.permute(0,2,1)
            res.append(out[:, :, :])  #torch.Size([32, 48, 3])

        res = torch.stack(res, dim=-1)  #torch.Size([32, 48, 3, 5])
        # adaptive aggregation
        period_weight = F.softmax(period_weight, dim=1)  #torch.Size([32,  5])

        # res 的时间步和特征维度
        _, T_res, N_res, K = res.shape  # torch.Size([32, 48, 3, 5])
        # 调整 period_weight 的形状
        period_weight = period_weight.unsqueeze(1).unsqueeze(1).repeat(1, T_res, N_res, 1)
        # period_weight = period_weight.unsqueeze(1).unsqueeze(1).repeat(1, T, N, 1)#(32, 5) 调整为 (32, T, N, 5)，使后续的计算中能与输入张量进行逐元素操作，同时通过广播机制正确地应用每个周期权重 torch.Size([32, 120, 3, 5])
        res = torch.sum(res * period_weight, -1)#最后一个维度上进行求和，这会将所有周期的加权结果相加
        # residual connection
        # x = res + x 
        x=res
        return x
...
class CARDformer(nn.Module):
    def __init__(self, 
        ...
        self.config = config  ##add2
        self.device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')##add2
        ...
        self.W_pos_embed = nn.Parameter(torch.randn(patch_num,config.d_model).to(self.device) *1e-2)
        ...
        self.W_input_projection = nn.Linear(self.patch_len, config.d_model).to(self.device)   
    ...
    def update_patch_len(self, patch_len):  #add2
            """
            动态更新 patch_len 及相关参数
            """
            self.patch_len = patch_len
            self.patch_num = int((self.config.seq_len - self.patch_len) / self.stride + 1)
            self.total_token_number = (self.patch_num  + self.model_token_number + 1)
            self.W_pos_embed = nn.Parameter(torch.randn(self.patch_num, self.d_model).to(self.device)  * 1e-2)
            self.W_out = nn.Linear((self.patch_num+1+self.model_token_number)*self.d_model, self.pred_len).to(self.device) 
            self.W_input_projection = nn.Linear(self.patch_len, self.d_model).to(self.device)   # 更新输入投影层    

    def forward(self, z,patch_len,*args, **kwargs):  

        # 动态更新 patch_len
        if patch_len != self.patch_len:  #add2
            self.update_patch_len(patch_len)  #add2
        ...
        zcube = z.unfold(dimension=-1, size=patch_len, step=self.stride).to(self.W_pos_embed.device)  #add2
...
class Attenion(nn.Module):
    def __init__(self,config, over_hidden = False,trianable_smooth = False,untoken = False, *args, **kwargs):
        ...
        # self.attn1 = Multi_GTU(num_of_timesteps=config.total_token_number, in_channels=config.d_model, time_strides=1, kernel_size=[3,5,7], pool=True)
        self.attn1 = Multi_GTU(num_of_timesteps=config.d_model, in_channels=self.c_in, time_strides=1, kernel_size=[3,5,7], pool=True)
        # self.attn1= CrossRegionAttntion(dim=config.d_model, num_heads=config.n_heads, region_size=5)
    ...
    def forward(self, src, *args,**kwargs):
        ...
        if not self.over_hidden:  #over token attn 2
            ...
            # B C H W
            # # X = src.permute(0,3,1,2)  #[32,7,12,512]->[32,512,7,12] 
            # X = src 
