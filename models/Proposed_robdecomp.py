...
class series_decomp_2diff(nn.Module):
    """
    Series decomposition block with 2-step moving average (for trend extraction)
    """
    def __init__(self, kernel_size):
        super(series_decomp_2diff, self).__init__()
        self.moving_avg = moving_avg(kernel_size, stride=1)
        self.layer = torch.nn.Linear(1, 3)  # Linear layer for 2-way difference
    ...
    def forward(self, x):
        ...
        moving_mean = self.moving_avg(x)
        # Second moving average (to capture second-order trend)
        moving_mean2 = self.moving_avg(moving_mean)
        moving_mean3 = self.moving_avg(moving_mean2)

        
        trend.append(moving_mean.unsqueeze(-1))  # Shape (1, L, 1)
        trend.append(moving_mean2.unsqueeze(-1))  # Shape (1, L, 1)
        trend.append(moving_mean3.unsqueeze(-1))  # Shape (1, L, 1)

        
       
        trend = torch.cat(trend, dim=-1)  # Concatenate along the last dimension (2 trends)

        # Applying softmax to learn the importance of each trend component
        trend = torch.sum(trend * nn.Softmax(-1)(self.layer(x.unsqueeze(-1))), dim=-1)

        # Compute the residual (seasonal component)
        res = x - trend  # Seasonal component
        return res, trend
...
class Model(nn.Module):  #add
    def __init__(self, config, **kwargs):
        ...
        self.decomp = series_decomp_2diff(kernel_size=config.moving_avg)
