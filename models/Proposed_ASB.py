...
class Adaptive_Spectral_Block(nn.Module):
    def __init__(self, adaptive_filter,dim):
        super().__init__()
    ...
    def forward(self, x_in):
        B, N, C = x_in.shape  #torch.Size([96, 2, 128])
...
class CARDformer(nn.Module):
    def __init__(self, 
        ...
        self.adaptive_filter = config.adaptive_filter
        ...
        self.asb = Adaptive_Spectral_Block(self.adaptive_filter ,config.d_model)

         #使用 linspace 来为每一层创建不同的丢弃概率
        dpr = [x.item() for x in torch.linspace(0, config.dropout, config.e_layers)]     
        # 初始化 DropPath 层
        self.drop_path = nn.ModuleList([
            DropPath(dpr[i]) if dpr[i] > 0. else nn.Identity() for i in range(config.e_layers)
        ])
    ...
    def forward(self, z,*args, **kwargs):  
        ...
        z_embed = rearrange(z_embed, 'b m n p -> (b m) n p')  #torch.Size([96, 14, 128])
        z_embed = z_embed + self.drop_path[1](self.asb(self.norm1(z_embed)))#add asb 
        z_embed = rearrange(z_embed, '(b m) n p -> b m n p', b=b)
...
#             output_along_token=output.reshape(B,nvars, self.n_heads,H, C // self.n_heads)  #torch.Size([32, 3, 8, 15, 16])
...
#             # 1. 调整 q, k, v 的形状，使 Token 数量变为计算注意力的主要维度add
#             q = q.permute(0, 1, 2, 4, 3)  # Shape: [32, 12, 7, 8, 64]   torch.Size([32, 15, 3, 8, 16])
#             k = k.permute(0, 1, 2, 4, 3)  # Shape: [32, 12, 7, 8, 64]
#             v = v.permute(0, 1, 2, 4, 3)  # Shape: [32, 12, 7, 8, 64]
...
#             # output_along_hidden=output_along_hidden.reshape(B,H, self.n_heads,nvars, C // self.n_heads)
...
#         if not self.untoken:
#         # if not self.untoken and self.over_hidden:
